...
// TODO: wire these up to real endpoints once backend routes exist
const [announcements] = useState([]);
const [performance] = useState({ average: null, rank: null });
const [homework] = useState([]);
const [results] = useState([]);

const pendingHomework = useMemo(
  () => homework.filter((item) => !['submitted', 'reviewed'].includes(item.submission_status)),
  [homework]
);
...
const upcomingLectures = useMemo(() => {
  const now = new Date();
  return lectures
    .filter((l) => new Date(l.scheduled_at || l.scheduledAt) > now)
    .slice(0, 3);
}, [lectures]);
...
<View style={{ height: 420, marginBottom: 24 }}>
  ...
  />
        ...
        {/* Weak Topics — hidden for this release, placeholder data only, not wired to backend yet
        <View style={styles.topicsSection}>
          <Text style={styles.subTitle}>Weak Topics</Text>
          <View style={styles.topicItem}>
            <View style={[styles.dot, { backgroundColor: '#ef4444' }]} />
        ...
        */}
        ...
        {/* Recommended Practice — hidden for this release, placeholder data only, not wired to backend yet
        <View style={styles.practiceSection}>
          <Text style={styles.subTitle}>Recommended Practice</Text>
          <TouchableOpacity style={styles.practiceItem}>
            <BookOpen size={20} color="#2b58ed" />
