...
import { UserSessionProvider, useUserSession } from './src/context/UserSessionContext';
      ...
      try {
        ...
        if (token) {
          ...
          if (profile?.role) {
            ...
            setInitialRoute(roleRouteMap[profile.role] ?? 'SDCLogin');
          }
        }
      } catch (e) {
        // Token invalid or network error — fall through to SDCLogin
      } finally {
        setIsLoading(false);
      }
    };
